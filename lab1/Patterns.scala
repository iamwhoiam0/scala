...
object Patterns extends App {
  ...
  def winsA(a: Hand, b: Hand): Result = a match {
    case Rock => b match {  // Камень
      case Rock => Draw
      case Scissor => Win
      case Paper => Lose
    }
    case Scissor => b match {  // Ножницы
      case Scissor => Draw
      case Paper => Win
      case Rock => Lose
    }
    case Paper => b match { // Бумага
      case Paper => Draw
      case Rock => Win
      case Scissor => Lose
    }
  ...
  println(testExtractMammalWeight(Mammal("Fox", Meat, 1414)))
